...
func NewWhatsAppAdapter(cfg config.WhatsAppChannelConfig, appendEvent func(eventType string, sessionID string, payload map[string]any)) *WhatsAppAdapter {
	return &WhatsAppAdapter{
		base:        inputlayer.NewBaseAdapter("whatsapp", cfg.Enabled && cfg.AccessToken != "" && cfg.PhoneNumberID != ""),
		config:      cfg,
...
func (a *WhatsAppAdapter) Status() inputlayer.Status {
	status := a.base.Status()
	status.Enabled = a.Enabled()
	return status
}

func (a *WhatsAppAdapter) Run(ctx context.Context, handle inputlayer.InboundHandler) error {
...
func (a *WhatsAppAdapter) HandleInbound(ctx context.Context, source string, text string, messageID string, profileName string, handle inputlayer.InboundHandler) (string, string, error) {
...
func (a *WhatsAppAdapter) pruneSeenLocked() {
...
func (a *WhatsAppAdapter) hasSeen(id string) bool {
